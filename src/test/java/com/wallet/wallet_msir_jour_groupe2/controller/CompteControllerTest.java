...
class CompteControllerTest {
    ...
    void list() {
       /* Model model = mock(Model.class);
        ...
        List<CompteDTO> fakeList=new ArrayList<>();
        when(compteService.findAll()).thenReturn(fakeList);
        Model model= Mockito.mock(Model.class);
        String retour=compteController.list(model);
        assertEquals("compte/list",retour);
    }
    @Test
    void add() {
       /* CompteDTO compteDTO = new CompteDTO();
        ...
        CompteDTO compteDTO=Mockito.mock(CompteDTO.class);
        String ajout=compteController.add(compteDTO);
        assertEquals("compte/add",ajout);
    }
    ...
    void testAdd() throws Exception{
        ...
        String current_page = compteController.add(compteDTO, bindingResult, redirectAttributes);
        verify(compteService, times(1)).create(compteDTO);
        assertEquals("redirect:/comptes", current_page);
